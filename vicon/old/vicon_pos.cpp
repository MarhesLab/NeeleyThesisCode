...
vicon_pos::vicon_pos() {
	ViconConnect("192.168.0.112:801");
}

// Overloaded constructor so the IP can be changed without recompilling this class.
vicon_pos::vicon_pos(const char *IP)
{
	ViconConnect(IP);
}

void vicon_pos::ViconConnect(const char *VICONIP)
{
	...
	std::cout << "Final del constructor.....\n";
}
...
// William Neeley 9-05-2014
int vicon_pos::getTimeStampValues(unsigned int& hours, unsigned int& mins, unsigned int& secs)
{
	...
	Result::Enum timeStampResult = temp_timecode.Result;
	if(timeStampResult == Result::Success)
		return 0;
	if(timeStampResult == Result::NotConnected)
		return -1;
	if(timeStampResult == Result::NoFrame)
		return 1;
	return 2;
}

double vicon_pos::getLatencyTotal()
{
